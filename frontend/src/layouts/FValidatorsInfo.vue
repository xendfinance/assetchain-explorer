<template>
    <div class="f-validators-info">
        <template v-if="!dValidatorsInfoError">
            ...
            <div
                class="row f-data-layout equal-height no-vert-col-padding collapse-md"
            >
                <div class="col margin-bottom-menu">
                    <!-- <f-card>
                        <h2>
                            {{ $t("view_validators_info.staking_summary") }}
                        </h2>

                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{
                                    $t("view_validators_info.total_self_staked")
                                }}
                            </div>
                            <div class="col">
                                <div v-show="'fSelfStaked' in dTotals">
                                    {{ dTotals.fSelfStaked }} {{ symbol }}
                                    <span v-if="cSelfStaked"
                                        >({{ cSelfStaked }}%)</span
                                    >
                                </div>
                            </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.total_delegated") }}
                            </div>
                            <div class="col">
                                <div v-show="'fTotalDelegated' in dTotals">
                                    {{ dTotals.fTotalDelegated }} {{ symbol }}
                                    <span v-if="cDelegated"
                                        >({{ cDelegated }}%)</span
                                    >
                                </div>
                            </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.total_staked") }}
                            </div>
                            <div class="col">
                                <div v-show="'fTotalStaked' in dTotals">
                                    {{ dTotals.fTotalStaked }} {{ symbol }}
                                    <span v-if="cStaked">({{ cStaked }}%)</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.daily_rewards") }}
                            </div>
                            <div class="col">
                                <div v-show="cDailyRewards">
                                    {{
                                        formatNumberByLocale(
                                            numToFixed(cDailyRewards, 0)
                                        )
                                    }}
                                    {{ symbol }}
                                </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{
                                    $t(
                                        "view_validators_info.current_reward_rate"
                                    )
                                }}
                            </div>
                            <div class="col">
                                <div v-show="cCurrentRewardRate">
                                    {{
                                        formatNumberByLocale(
                                            numToFixed(cCurrentRewardRate, 0)
                                        )
                                    }}%
                                </div>
                            </div>
                ...
                <div class="col">
                    <!-- <f-card>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.epoch_number") }}
                            </div>
                            <div class="col">
                                <div v-show="'id' in cEpoch">
                                    {{ cEpoch.id | formatHexToInt }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.end_time") }}
                            </div>
                            <div class="col">
                                <div v-show="'endTime' in cEpoch">
                                    <timeago
                                        :datetime="
                                            timestampToDate(cEpoch.endTime)
                                        "
                                        :auto-update="1"
                                        :converter-options="{ addSuffix: true }"
                                    ></timeago>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.duration") }}
                            </div>
                            <div class="col">
                                <div
                                    v-show="
                                        'duration' in cEpoch && cEpoch.duration
                                    "
                                >
                                    {{
                                        formatHexToInt(cEpoch.duration)
                                            | formatDuration
                                    }}
                                </div>
                            </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.fee") }}
                            </div>
                            <div class="col">
                                <div v-show="'epochFee' in cEpoch">
                                    {{ WEIToFTM(cEpoch.epochFee) }} {{ symbol }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row no-collapse">
                            <div class="col-5 f-row-label">
                                {{ $t("view_validators_info.total_supply") }}
                            </div>
                            <div class="col">
                                <div v-show="dTotalSupply">
                                    {{
                                        formatNumberByLocale(
                                            numToFixed(dTotalSupply, 0)
                                        )
                                    }}
                                    {{ symbol }}
                                </div>
        ...
        <div v-if="showRewardsEstimation" class="f-subsection">
            <h2 class="h1">
                {{ $t("view_validators_info.estimate_your_rewards") }}
            </h2>
            <div
                class="row f-data-layout equal-height no-vert-col-padding collapse-md"
            >
        ...
        <div class="f-subsection" v-if="dInactiveItems.length">
            <h2 class="h1">
                {{ $t("view_validator_list.inactive") }}
                <span class="f-records-count"
                    >({{ dInactiveItems.length }})</span
                >
            </h2>

            <f-validator-list
                :items="dInactiveItems"
                code="inactive-validators"
            />
        </div>

        <div class="f-subsection" v-if="dOfflineItems.length">
            <h2 class="h1">
                {{ $t("view_validator_list.offline") }}
                <span class="f-records-count"
                    >({{ dOfflineItems.length }})</span
                >
            </h2>

            <f-validator-list
                :items="dOfflineItems"
                code="offline-validators"
            />
        </div>

        <div class="f-subsection" v-if="dFlaggedItems.length">
            <h2 class="h1">
                {{ $t("view_validator_list.flagged") }}
                <span class="f-records-count">
                    ({{ dFlaggedItems.length }})
                </span>
            </h2>

            <f-validator-list
                :items="dFlaggedItems"
                code="flagged-validators"
            />
        </div>
    </div>
</template>
